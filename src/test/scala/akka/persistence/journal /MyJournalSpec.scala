...
/**
  ...
  */
abstract class MyJournalSpec(config: Config)
    extends PluginSpec(config)
    with MayVerb
    with OptionalTests
    with JournalCapabilityFlags {

  implicit lazy val system: ActorSystem = ActorSystem("JournalSpec", config.withFallback(MyJournalSpec.config))
