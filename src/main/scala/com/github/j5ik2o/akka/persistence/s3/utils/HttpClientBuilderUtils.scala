...
object HttpClientBuilderUtils {
  def setup(clientConfig: S3ClientConfig): NettyNioAsyncHttpClient.Builder = {
    ...
    clientConfig.maxPendingConnectionAcquires.foreach(
      v => result.maxPendingConnectionAcquires(v)
    )
    clientConfig.readTimeout.foreach(
      v => result.readTimeout(JavaDuration.ofMillis(v.toMillis))
    )
    clientConfig.writeTimeout.foreach(
      v => result.writeTimeout(JavaDuration.ofMillis(v.toMillis))
    )
    ...
    clientConfig.useConnectionReaper.foreach(
      v => result.useIdleConnectionReaper(v)
    )
